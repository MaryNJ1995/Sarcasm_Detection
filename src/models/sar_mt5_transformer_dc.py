...
class Classifier(pl.LightningModule):
    def __init__(self, n_classes: int, class_weights, arg):
        ...
        self.enc_layer1 = EncoderLayer(hid_dim=self.model.config.hidden_size,
                                       n_heads=16, pf_dim=self.model.config.hidden_size * 2,
                                       dropout=arg.dropout)
        self.enc_layer2 = EncoderLayer(hid_dim=self.model.config.hidden_size * 2,
                                       n_heads=16, pf_dim=self.model.config.hidden_size * 4,
                                       dropout=arg.dropout)
        self.enc_layer3 = EncoderLayer(hid_dim=self.model.config.hidden_size * 4,
                                       n_heads=16, pf_dim=self.model.config.hidden_size * 8,
                                       dropout=arg.dropout)
        self.output_layer = torch.nn.Linear(in_features=8 * self.model.config.hidden_size,
                                            out_features=n_classes)
        self.max_pool1d = nn.MaxPool1d(arg.max_length)
        self.attention = ScaledDotProductAttention(dim=8 * self.model.config.hidden_size)
    ...
    def forward(self, batch):
        ...
        enc_out1 = self.enc_layer1(token_features)
        # enc_out.shape:[batch_size, seq_len, 2*emb_dim] = ([32, 150, 1024])

        enc_out2_input = torch.cat((enc_out1, token_features), dim=2)
        # enc_out3_input.shape: [batch_size, seq_len, 4 * emb_dim] = ([32, 150, 2 * 1024])

        enc_out2 = self.enc_layer2(enc_out2_input)
        # enc_out2.shape:[batch_size, seq_len, 2*emb_dim] = ([32, 150, 2*1024])

        enc_out3_input = torch.cat((enc_out1, enc_out2, token_features), dim=2)
        # enc_out3_input.shape: [batch_size, seq_len, 2 * emb_dim] = ([32, 150, 4 * 1024])

        enc_out3 = self.enc_layer3(enc_out3_input)
        # enc_out3_input.shape: [batch_size, seq_len, 2 * emb_dim] = ([32, 150, 4 * 1024])

        out_features = torch.cat((enc_out3, enc_out2, enc_out1, token_features), dim=2)
        # out_features.shape: [batch_size, seq_len, 4*emb_dim] = ([32, 150, 8 * 1024])

        context, attn = self.attention(out_features, out_features, out_features)
        context = context.permute(0, 2, 1)
        # context.size= [batch_size, feature_dim, sent_len] ===>torch.Size([32,  8 * 1024], 150])
