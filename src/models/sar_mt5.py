...
class Classifier(pl.LightningModule):
    def __init__(self, n_classes: int, class_weights, arg):
        ...
        self.output = torch.nn.Linear(in_features=self.mt5_model.arg.hidden_size,
    ...
    def forward(self, batch):
        mt5_output = self.mt5_model(batch["inputs_ids"]).last_hidden_state.permute(0, 2, 1)
        # mt5_output.size() = [batch_size, sequence_length, hidden_size]

        pooler = self.pooler(mt5_output).squeeze(2)
